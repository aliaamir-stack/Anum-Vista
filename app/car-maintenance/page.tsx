...
const EXTRA_MAINTENANCE_PER_ADDITIONAL_CAR = 2000;
...
export default function CarMaintenancePage() {
  ...
  const saveRow = async (tenant: Occupant) => {
    ...
    if (Number.isNaN(normalizedCars)) {
      setError("Please enter a valid number of cars.");
      return;
    }
    ...
    try {
      ...
      const updated = await api.updateCarMaintenance(tenant.id, {
        ...
        extra_car_maintenance_fee: EXTRA_MAINTENANCE_PER_ADDITIONAL_CAR.toFixed(2),
      });
    ...
    <section className="space-y-6 p-8">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">
          1 car = no additional maintenance. Each extra car after the 1st adds Rs{" "}
          {EXTRA_MAINTENANCE_PER_ADDITIONAL_CAR.toLocaleString("en-PK")} to the monthly maintenance.
        </p>
      </div>
        ...
        <table className="min-w-full text-left text-sm">
          <thead style={{ backgroundColor: "#1e2a3a", color: "#ffffff" }}>
            <tr>
              ...
              <th className="px-5 py-3 font-semibold">Expected Extra (rule)</th>
          ...
          <tbody>
            {isLoading
              ...
              : tenants.map((tenant, index) => {
                  const editor = editors[tenant.id];
                  const computedCars = Math.max(0, Number(editor?.carCount ?? 0));
                  const additionalCars = Math.max(0, computedCars - 1);
                  const expectedExtra =
                    additionalCars * EXTRA_MAINTENANCE_PER_ADDITIONAL_CAR;
                  return (
                    <tr key={tenant.id} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                      ...
                      <td className="px-5 py-4">
                        ...
                        <p className="mt-1 text-xs text-slate-500">
                          Extra cars: {additionalCars} → +Rs{" "}
                          {expectedExtra.toLocaleString("en-PK")}
                        </p>
                      </td>
                      <td className="px-5 py-4">
                        <input
                          type="text"
                          value={`Rs ${EXTRA_MAINTENANCE_PER_ADDITIONAL_CAR.toLocaleString(
                            "en-PK",
                          )}`}
                          disabled
                          className="w-32 cursor-not-allowed rounded border border-slate-200 bg-slate-50 px-2 py-1 text-sm text-slate-700"
                        />
                      </td>
                      <td className="px-5 py-4 text-slate-700">
                        Rs {expectedExtra.toLocaleString("en-PK")}
                      </td>
                      <td className="px-5 py-4 text-slate-700">
